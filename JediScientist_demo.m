...
try
    ...
    uno = serial('COM16','BaudRate',115200); %setup connection to arduino
...
THRESH = 0.5;   % MAKES CONTROL BINARY
GAIN = [4,2];  % INCREASE VALUES TO INCREASE GAIN ON THE CHANNELS
NOISE = [0.35,0.35]; % INCREASE VALUES TO ACCOMODATE NOISEY CHANNELS
...
while(toc < Tmax && ishandle(fig)) %run until time is out or figure closes
    ...
    if(timeStamp > previousTimeStamp + delay) %limit rate
        ...
        try
            dataWindow = data(samp-300:samp,:); %grab last 300 datapoints
            dataWindow = data(samp-300:samp,:); %grab last 300 datapoints
            D = mean(abs(dataWindow-mean(dataWindow))); %calculate mean absolute value
            D(1) = GAIN(1)*D(1)-NOISE(1);
            D(2) = GAIN(2)*D(2)-NOISE(2);
            D(D>1) = 1;
            D(D<=0) = 0;    
            D(isnan(D)) = 0;
            control(samp,2) = D(2);
